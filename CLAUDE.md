...
- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — tech stack, patterns (tool registry, process model, IPC, error handling), testing approach, file conventions. Read at the start of any non-trivial task.
- [docs/DECISIONS.md](docs/DECISIONS.md) — running log of noteworthy choices, caveats, recipes. Check when something looks weird; it's probably explained there.
- [docs/ADDING_A_TOOL.md](docs/ADDING_A_TOOL.md) — playbook for adding a new tool to the registry. Read before starting any tool.
- [docs/plans/BACKLOG.md](docs/plans/BACKLOG.md) — forward-looking plans and ideas not yet committed to a milestone.
- [docs/plans/](docs/plans/) — **ephemeral per-tool working docs** (one per in-flight tool, deleted when the tool ships). No persistent per-tool documentation — tools should be self-describing in code.
- [CHANGELOG.md](CHANGELOG.md) — per-release notes (generated from conventional commits via `git-cliff`).

If a request conflicts with ARCHITECTURE, surface the conflict rather than silently picking one — that doc changes through discussion, not drift.

## Working rules

### Architecture
- New tools are added as self-contained modules following the registry pattern in [ARCHITECTURE §3.1](docs/ARCHITECTURE.md#31-tool-registry-pattern). Do not edit shared dashboard/routing code to add a tool.
...
- No `unwrap()` or `expect()` in non-test code; use typed `Result<T, AppError>` variants (see [ARCHITECTURE §3.4](docs/ARCHITECTURE.md#34-error-handling))
...
### Testing
- Add or update tests with every behavioral change. Coverage target on Rust processing modules is ≥80% (see [ARCHITECTURE §4](docs/ARCHITECTURE.md#4-testing-strategy)).
...
### Scope discipline
- Don't add features, refactors, or abstractions beyond the task at hand. If ARCHITECTURE doesn't call for it, ask before building it.
...
1. From `src-tauri/`: `cargo fmt --all --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test -p multitool-core --all-targets`
   - `--workspace` so clippy covers both the shell and `multitool-core`; `-p multitool-core` for `cargo test` because the Tauri shell's test exe fails to launch on the Windows CI runner (see [DECISIONS.md](docs/DECISIONS.md) → "Workspace split").
...
`-p multitool-core` on `cargo test`/`cargo llvm-cov` keeps the Tauri-shell test exe out of the run; it can't launch on the Windows CI runner (see [DECISIONS.md](docs/DECISIONS.md) → "Workspace split").
...
- Don't write to disk silently. The output-naming and duplicate-handling rules in [ARCHITECTURE §3.3](docs/ARCHITECTURE.md#33-file-io-conventions) are not optional.
