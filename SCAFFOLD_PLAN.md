...
React 18 per ARCHITECTURE §1 (template defaulted to 19), dropped `tauri-plugin-opener`
...
uses hash mode per ARCHITECTURE §1. `tsconfig.json` gained `noUncheckedIndexedAccess`
...
prettier-ignored. The directory skeleton from ARCHITECTURE §6
...
crates wait for the first tool. `AppError` (ARCHITECTURE §3.4 variants) lives in
`error.rs` and serializes as `{ kind, message }` so the webview can branch on
`kind`. Module skeleton from ARCHITECTURE §2 in place: `tools/`, `ipc/`, `fs/`.
...
The fix realigns the crate layout with ARCHITECTURE §3.1 ("pure functions, testable
...
## Phase H — Repo polish — DONE in `docs: scaffold readme, changelog, commands cheat sheet, doc reorg`

Replaced the create-tauri-app README template with a one-paragraph intro,
docs index, quickstart, and project-layout map naming the `multitool-core`
rlib; CI badge wired to the master workflow. `CHANGELOG.md` ships with just
the cliff.toml header (`# Changelog ...`); release tags will populate it
via `git cliff --unreleased --prepend CHANGELOG.md`. The CLAUDE.md commands
cheat-sheet placeholder is filled in with the real commands grouped by
purpose, including the `-p multitool-core` quirk on `cargo test`. **Doc
reorg:** the original `SPEC.md` was a mix of product brief and tech
specification; split into [ASSIGNMENT.md](ASSIGNMENT.md) (product brief,
acceptance, roadmap — retires after Phase 1 ships) and
[ARCHITECTURE.md](ARCHITECTURE.md) (tech stack, patterns, IPC contracts,
testing, CI). Seeded [DECISIONS.md](DECISIONS.md) with ~12 entries
(workspace split, default-members revert, GA Node-24 bump, pnpm
packageExtensions hack, ESLint 9 pin, Tailwind v4, Playwright vs
WebdriverIO, llvm-cov vs tarpaulin, lefthook choice, VS Code snap caveat,
branch-protection shape). CLAUDE.md drops the duplicated tech-stack and
project-intent blocks; replaced with a docs index that points to the new
files. `LICENSE` stays intentionally absent per the project-intent rule.

---

## Phase I — Tag

1. **Migrate persistent technical notes from this file to [DECISIONS.md](DECISIONS.md)** so they survive deletion. Mainly the Phase A dev-env workaround and any Phase-specific gotchas not already captured (most are).
2. Open a single PR (or merge the checkpoint commits) into `master`.
3. After merge: tag `v0.1.0-scaffold` on `master`. This is the baseline for feature work; the actual `v0.1.0` waits until both Phase 1 tools ship.
4. **Delete this file.** Its persistent content has moved to DECISIONS.md / ARCHITECTURE.md.
...
- `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo test -p multitool-core --all-targets` pass with zero warnings.
- `pnpm lint`, `pnpm typecheck`, `pnpm test` pass.
- Lefthook pre-commit blocks a deliberately-bad commit (manual sanity check).
- CI passes on all three OSes for a no-op PR (validated via PR #1).
- Adding a new tool requires only: one folder under `src/tools/`, one folder under `src-tauri/src/tools/`, one import line in each registry. No edits to shared shell/routing code ([ARCHITECTURE §3.1](ARCHITECTURE.md#31-tool-registry-pattern)).

---

## Explicit non-goals for the scaffold PR

To keep the scaffold work reviewable and avoid scope creep:

- No feature code (no PDF↔image logic).
- No drag-and-drop input ([ASSIGNMENT §4](ASSIGNMENT.md#4-ux-principles) marks it as a Phase 2 slip).
