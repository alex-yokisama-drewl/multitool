...
| Image decode (EXIF-aware) | [`multitool_core::image`](../src-tauri/multitool-core/src/image/mod.rs) | `decode_oriented(source_ext, bytes) -> AppResult<DynamicImage>` applies any EXIF orientation tag the decoder exposes; falls back to extension-based format detection when bytes-sniffing is inconclusive (TGA has no magic). `image_to_app_err(err)` is the companion variant mapper. `images_to_pdf`, `image_format_converter`, and `image_crop` route through this. |
| Encodable raster format set | [`multitool_core::image::RasterFormat`](../src-tauri/multitool-core/src/image/raster_format.rs) | The formats the pipeline can both decode AND encode (PNG/JPEG/WebP/BMP/TIFF) + per-variant metadata (`extensions`, `default_extension`, `image_format`, `supports_alpha`, `from_extension`). Served to the frontend by the `supported_raster_formats` command. Don't hard-code this list — add a variant here and consumers follow. |
| Supported-format query (IPC) | `crate::system::supported_raster_formats` + [`src/lib/imageFormats.ts`](../src/lib/imageFormats.ts) | Backend-served `RasterFormatDescriptor[]`; frontend reads via the memoized `getRasterFormats()` / `rasterImageExtensions()`. The picker filters + the converter's output dropdown consume this so the format list lives in exactly one place (Rust). |
...
| System-level OS wrappers | [`src/lib/system.ts`](../src/lib/system.ts) | `pickPdfFile()`, `revealInFolder(path)`, `pickImageFiles()` / `pickConvertibleImages()` / `pickRasterImage()`, `imageAssetUrl()`, `allowMediaPreview()`. Extend here when a tool needs picker / dialog / opener; don't import `@tauri-apps/plugin-*` from components. |
