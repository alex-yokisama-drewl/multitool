...
| Shared components (FilePicker, JobProgress, …) | [../src/components/](../src/components/) | None yet — bias toward inlining in the tool folder until the same shape shows up in a second tool. Staging-grid thumbnail cards are inlined per-tool today (deliberately — see [../DECISIONS.md](../DECISIONS.md) → "Staging-grid thumbnail card stays inline"). |
