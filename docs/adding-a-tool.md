...
| System-level OS wrappers | [`src/lib/system.ts`](../src/lib/system.ts) | `pickPdfFile()`, `revealInFolder(path)`. Extend here when a tool needs picker / dialog / opener; don't import `@tauri-apps/plugin-*` from components. |
