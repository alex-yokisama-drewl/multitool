...
| # | Status | Commit | Scope |
| --- | --- | --- | --- |
| 1 | done — [`THIS-COMMIT`] | `docs(audio-extractor): plan` | Working doc (brief + this plan). |
| 2 | pending | `feat(core): probe_audio_stream_count` | Add `probe_audio_stream_count(source) -> AppResult<u32>` to [`multitool_core::ffmpeg`](../../src-tauri/multitool-core/src/ffmpeg.rs), parsing `ffmpeg -i` stderr for `Stream #...: Audio:` lines. Unit tests on 0 / 1 / N tracks + garbage input via the bundled ffmpeg's `lavfi` testsrc (same trick the Video Format Converter tests use to synth real fixtures). Lives in `multitool_core` so the existing Video Format Converter's `probe_duration_secs` can sit next to it. |
| 3 | pending | `feat(audio-extractor): pure-logic extraction` | Create `multitool-core/src/tools/audio_extractor/{convert.rs, job.rs, mod.rs}`. `convert::extract_one_track(source, track_index, track_total, on_progress, cancel) -> AppResult<PathBuf>` owns the per-track ffmpeg invocation + naming + partial-file cleanup. `job::run_job(source, cancel, on_progress) -> AppResult<JobResult>` probes track count, loops, emits `Progress::Started / FileProgress / Succeeded` (no `Skipped` — single-file shape, no per-track recoverable failure mode). Register `audio_extractor` in [`multitool-core/src/tools/mod.rs`](../../src-tauri/multitool-core/src/tools/mod.rs). Tests: single-track happy, multi-track happy (synthesize multi-stream input via `lavfi` `testsrc` + two `sine` generators), no-audio-source, missing-input, cancel-before, cancel-between-tracks, cancel-mid-track, collision through `unique_path`, on-progress error aborts. |
| 4 | pending | `feat(audio-extractor): Tauri command shim` | Add `src-tauri/src/tools/audio_extractor/mod.rs` with `#[tauri::command] extract_audio(app, job_id, path, registry) -> Result<JobResult, AppError>` — thin wrapper over `crate::ipc::run_streaming_job` calling `run_job` (no per-tool `Opts` to thread through). Register `pub mod audio_extractor;` + extend `generate_handler!` in [`src-tauri/src/tools/mod.rs`](../../src-tauri/src/tools/mod.rs). |
| 5 | pending | `feat(audio-extractor): TS picker + IPC wrapper` | Add `pickVideoFile()` (singular) to [`src/lib/system.ts`](../../src/lib/system.ts) — same extension filter as `pickVideoFiles`, `multiple: false`. Add `src/lib/tools/audioExtractor.ts` mirroring the Rust `Progress` + `JobResult` shapes; `extractAudio(path, hooks)` runs the job via `runJob`. Vitest covers: invokes `extract_audio`, JobId filtering, listener cleanup on both arms, AbortSignal → `cancel_job`. |
