...
| 6 | done — `0a31ca1` | `feat(audio-extractor): React UI + register` | Full tool folder + Dashboard.test updates landed. State machine matches plan. Multi-track "Track N of M" label conditional on `total > 1`. Done view lists each output's filename; reveal targets `outputs[0]`. Tile under Video, amber color. Nothing surprising. |
