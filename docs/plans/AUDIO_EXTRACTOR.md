...
| 6 | done — `0a31ca1` (+ style fixup `56bcd16`) | `feat(audio-extractor): React UI + register` | Full tool folder + Dashboard.test updates landed. State machine matches plan. Multi-track "Track N of M" label conditional on `total > 1`. Done view lists each output's filename; reveal targets `outputs[0]`. Tile under Video, amber color. **Gotcha:** forgot to run `pnpm format:check` before committing — lefthook caught it on the next staged change. Cost: one follow-up `style(...)` commit. Adding format:check to my pre-commit verification routine. |
| 7 | done — `e0e8ad2` | `test(audio-extractor): Playwright happy-path e2e` | One spec at `tests/e2e/audio-extractor.spec.ts`; mock at `tests/e2e/mocks/audioExtractor.ts` returning track_count = 2 so the running view also exercises the multi-track label. Vite alias extended; `pickVideoFile` added to the e2e system mock. Spec passes in 442 ms. |
