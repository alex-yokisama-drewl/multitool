...
| 3 | `feat(audio): pure-Rust WAV + FLAC encoders` — `encode_wav` (hound), `encode_flac` (flacenc). Round-trip tests (decode the encoded output, verify dims). | **done** — TBD-SHA. Two non-obvious findings: **(a)** `flacenc` 0.5 doesn't expose a single compression knob — only fine-grained `subframe_coding` / `stereo_coding` blocks; `Opts::flac_compression_level` is kept forward-compat but is a no-op in v1 (test asserts levels 0 and 8 produce identical bytes). **(b)** Symphonia 0.6's FLAC demuxer rejects flacenc-produced FLAC with "unexpected end of file" because it strictly checks STREAMINFO `total_samples` against demuxed frame count. Fix: added `claxon` 0.4 as a runtime dep, routed `decode_to_pcm` to claxon when the source has `fLaC` magic bytes or `source_ext == "flac"`; Symphonia stays for every other format. Existing Symphonia-decoded FLAC fixture (from ffmpeg) still passes — claxon also decodes it cleanly. 17 audio tests pass. |
