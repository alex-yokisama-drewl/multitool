...
| 2 | `feat(audio-trim): pure trim + fade` — `convert.rs::trim_and_fade(buf, start_ms, end_ms, fade_in_ms, fade_out_ms) -> AppResult<(AudioBuffer, Vec<String>)>`. Unit tests on each case (range correctness, fade math, overlap clamp warning, multi-channel). _Gotcha: the fade-in formula uses `gain = i / N` so `gain(0) == 0` literally — tests assert `out.samples[0] == 0` and `out.samples[N-1] == (N-1)/N`. If you switch to `gain = i / (N-1)` later, the boundary tests need to flip to `out.samples[N-1] == 1.0`._ | done (commit pending) |
