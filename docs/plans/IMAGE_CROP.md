...
| 7 | `feat(image-crop): TS IPC wrapper` | `src/lib/tools/imageCrop.ts` + `imageCrop.test.ts`. Pattern: JobId via `crypto.randomUUID()`, `invoke<JobResult>("convert_image_crop", { jobId, path, opts })`, `AbortSignal` → `cancel_job`, `try/finally` listener cleanup. Tests: invokes the right command, error envelope round-trip, AbortSignal aborts, no listener leak on error. | done | `b142134` | Command is `crop_image` (not `convert_image_crop`); `cropImage(path, rect)` sends `{ path, opts: rect }` so opts == the CropRect, matching the shell. Routes through the shared `runJob` helper (no bespoke listen/invoke). 5 tests green. |
| 8 | `feat(image-crop): React component with frame editor` | `src/tools/image-crop/{index.ts, ImageCrop.tsx, types.ts, ImageCrop.test.tsx}`. State machine `idle → picked → cropping → done | error`. New `pickRasterImage()` in `src/lib/system.ts` (single-select, awaits `rasterImageExtensions()` for the filter). `<img>` rendered via `imageAssetUrl(path)` (requires `allowMediaPreview` for the picked path). Frame overlay: corners/sides/body drag handlers + numeric inputs bidirectionally bound, proportional-resize checkbox captures ratio at toggle-on. Vitest: defaults render, options forwarded, error envelope renders, cancel aborts. | done | `4e0077e` | Extracted the clamp/aspect-lock math into a pure `cropGeometry.ts` (+20 unit tests) so `CropFrame.tsx` only does pointer→source-px mapping. Natural dims come from the `<img>` onLoad (frame withheld until then). Checkbox labelled "Lock proportions" (`color: violet`, distinct from converter's sky in the image category). **Test gotcha:** jsdom has no ResizeObserver → class stub with comment-bodied methods (a `vi.fn(()=>…)` stub isn't `new`-able); fire `<img>` load with `Object.defineProperty(naturalWidth/Height)` to reach the interactive state. NOT yet registered (that's #9). |
| 9 | `chore(image-crop): register tool + dashboard tile` | Add import + entry in `src/tools/registry.ts`; update `src/app/Dashboard.test.tsx` to assert the new tile. Only edits to shared frontend files. | done | `b2f5fa2` | Tile name "Crop"; the tile's accessible name includes the description ("Crop an image…"), so the dashboard assertion matches `/crop/i` (substring), not `/^crop$/i`. Image section now has 2 tiles. |
...
| 11 | `test(image-crop): playwright happy-path e2e` | `tests/e2e/image-crop.spec.ts` — dashboard → tile → pick (mocked) → crop → success state visible. New e2e mock under `tests/e2e/mocks/` if `imageCrop.ts` needs one. | done | `2efc825` | Added `mocks/imageCrop.ts` + vite alias + `pickRasterImage` stub. **Key gotcha:** the frame editor gates on the preview `<img>` onLoad (natural dims), so the mock `imageAssetUrl` had to switch from a 404-ing `mock-asset://` URL to a real 1×1 PNG **data URL** — otherwise Crop never enables in a plain browser. Full e2e suite (8 + 1 skipped) green. |
