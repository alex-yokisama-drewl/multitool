...
| --- | --- | --- | --- |
| 1 | in-progress | `build(video): bundle ffmpeg as Tauri sidecar resource` | `FFMPEG_TAG = "b6.1.1"` pin in `multitool-core/build.rs` + `src-tauri/build.rs`. Per-OS bare-binary download from eugeneware/ffmpeg-static into `OUT_DIR` (multitool-core) + staged copy under `resources/ffmpeg/` (src-tauri). `needs_copy` guard mirrors the pdfium dev-watcher fix. `tauri.conf.json` → `bundle.resources` extended to `resources/ffmpeg/*`; `.gitignore` extended to exclude staging dir. `FFMPEG_BIN_PATH` env override mirrors `PDFIUM_LIB_PATH` for offline / CI cache. No archive extraction needed — eugeneware ships bare binaries. **Gotchas discovered:** (a) eugeneware's `b6.1.1` tag actually contains ffmpeg 7.0.2 binaries on Linux (re-distributed from johnvansickle), not 6.1.1 as the tag suggests; codec set is what we need (libx264/x265/vpx/opus/lame/vorbis). (b) `fs::copy` preserves Unix mode bits, so re-applying `chmod 0o755` after the staging copy is defensive-only — kept it in for the `FFMPEG_BIN_PATH` override path where the source mode may be 0644. |
| 2 | pending | `feat(video): ffmpeg shim — spawn, progress parser, cancel` | New `multitool-core/src/ffmpeg.rs` (mirroring `pdfium.rs`'s "process-wide handle" shape but for an exec path). `init(path)` registers the resolved binary path; `run(args, on_progress, cancel) -> AppResult<()>` spawns the child with `-progress pipe:1 -nostats -hide_banner -loglevel error`, drains stdout line-by-line for the `out_time_us` key, throttles `on_progress` callbacks to ~250ms (audio precedent), keeps a 64-line stderr ring buffer for failure detail, kills + reaps on cancel. Unit tests on the line parser (pure logic). One integration test that runs `ffmpeg -version` to prove spawn works. Wire `init()` from `multitool/src/lib.rs::run` next to the pdfium init. |
| 3 | pending | `feat(video): convert + orchestrator (single + batch)` | `multitool-core/src/tools/video_format_converter/{mod.rs,convert.rs,job.rs}`. `TargetFormat::{Mp4,Webm,Mkv}` + recipe table → `Vec<String>` arg builder (pure, fully unit-tested). `convert(input, opts, on_progress, cancel)` calls `ffmpeg::run` with the built args. `job.rs` orchestrates the batch: probe duration once (via `ffmpeg -i` stderr parse, or a `ffprobe`-style invocation if we bundle ffprobe too — see gotcha below), loop over files, per-file skip on `ProcessingFailed`, `unique_path` for output naming, delete partial output on cancel before returning `Cancelled`. Integration tests generate synthetic clips and round-trip through each format. |
| 4 | pending | `feat(video): Tauri command + TS IPC wrapper` | `src-tauri/src/tools/video_format_converter/mod.rs` — `#[tauri::command] convert_video_format` using `crate::ipc::run_streaming_job`. Add to `register_commands` in `src-tauri/src/tools/mod.rs`. New `pickVideoFiles()` in `src/lib/system.ts` (dialog filter on the 5 extensions). New `src/lib/tools/videoFormatConverter.ts` IPC wrapper following the per-tool pattern (UUID JobId, `tool:progress` listener filtered by JobId, `AbortSignal` → `cancel_job`, `try/finally` unlisten). Vitest on the wrapper. |
