...
| — | **STOP — manual smoke session #1 (per ADDING_A_TOOL step 8)** | — |
| 9 | `fix(video-trimmer): WebM/VP9 preview proxy + minimal player controls` | done (SHA in #10) — **smoke #1 found preview broken**: WebKitGTK (Linux WebView) can't decode H.264/AAC without proprietary gstreamer plugins, so the native mp4 preview *and* the H.264 mp4 proxy both failed (crossed-out play, "error"). Fix: proxy is now **WebM/VP9/Opus** (`-deadline realtime -cpu-used 8 -crf 32`), open codecs WebKitGTK ships with — dest `…preview-{job_id}.webm`, cleanup guard updated. UI: dropped native `<video controls>` (inconsistent fullscreen/speed/download across WebViews) for a minimal bar — play/pause + seek slider + time + volume; removed the play-the-window button; moving a marker now pauses + reset-seeks. |
| 10 | `fix(video-trimmer): play preview from a blob URL, not the asset URL` | done `b82e528` — **smoke #2: WebM proxy still failed** — `play()` threw `NotSupportedError`. Root cause: WebKitGTK's `<video>` media pipeline (GStreamer) can't load Tauri's custom asset-protocol scheme at all, regardless of codec — but `fetch()` on the asset URL works (the audio trimmer proves it). Fix: **always** transcode the proxy, `fetch` its bytes, and play from `URL.createObjectURL(blob)`. Dropped native-first + `probePlayable` (deleted `src/lib/videoPreview.ts`) and `allowMediaPreview`. Blob/proxy torn down on pick/reset/unmount. |
| — | **STOP — manual smoke session #3** | — |
| 11 | `fix(video-trimmer): clamp preview to trim window, cap height, sweep stale proxies` | done (SHA in #12) — **smoke #3 polish**: (a) playback + seek slider + time readout now clamped to `[start,end]` (play starts at start, pauses at end; readout is window-relative); (b) removed the scrubber playhead line (the seek bar already shows position); (c) `<video>` capped at `max-h-[40vh] object-contain` so vertical clips stay small with controls in view; (d) temp leak — a hard app-close skips React unmount cleanup, so the active proxy survived. Added `cleanup_stale_proxies` command (sweeps all `multitool-preview-*.webm` in temp) called on tool mount. |
| — | **smoke #4 passed** ("all good"). Follow-up tweak in #12: dropped the wordy "Preparing preview" explainer (inaccurate now that every pick transcodes), kept just the heading + filename + progress. | — |
| 12 | `test(video-trimmer): Playwright happy-path e2e` (+ preparing-message trim) | done (SHA in #13) — `tests/e2e/video-trimmer.spec.ts` (dashboard route → pick → proxy → picked → trim → done); new `tests/e2e/mocks/videoTrimmer.ts`; `videoAssetUrl` added to the e2e system mock (tiny `data:` URL so the blob `fetch` resolves in plain Chromium); alias wired in `vite.config.ts`. Also trimmed the wordy "Preparing preview" explainer to heading + filename + progress. |
| 13 | `chore(video-trimmer): ship — DECISIONS entry, backlog, drop working doc` | pending |
