...
| 10 | `fix(video-trimmer): play preview from a blob URL, not the asset URL` | done (SHA in #11) — **smoke #2: WebM proxy still failed** — `play()` threw `NotSupportedError`. Root cause: WebKitGTK's `<video>` media pipeline (GStreamer) can't load Tauri's custom asset-protocol scheme at all, regardless of codec — but `fetch()` on the asset URL works (the audio trimmer proves it). Fix: **always** transcode the proxy, `fetch` its bytes, and play from `URL.createObjectURL(blob)`. Dropped native-first + `probePlayable` (deleted `src/lib/videoPreview.ts`) and `allowMediaPreview` (we never load the source in the WebView now). Blob/proxy torn down on pick/reset/unmount. Tradeoff: every pick transcodes + holds the proxy in memory as a blob (fine for the downscaled webm; note long-source cost in BACKLOG). Test: stub `fetch` + `URL.createObjectURL`; assert proxy→fetch→blob path. |
| — | **STOP — manual smoke session #3 (re-verify preview after the blob fix)** | — |
| 11 | `test(video-trimmer): Playwright happy-path e2e` | pending |
| 12 | `chore(video-trimmer): ship — DECISIONS entry, backlog, drop working doc` | pending |
