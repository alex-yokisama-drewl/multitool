...
| 4 | `feat(video-trimmer): core job orchestrator + duration probe + tests` | done (SHA in #5) — `job.rs` single-file `run_job` (`Progress::Started`/`FileProgress`, `JobResult{output,duration_ms}`); `probe.rs` `probe_duration_ms`. Cov: convert.rs 92%, job.rs 94.6%, probe.rs needed its own synth-clip happy-path test to clear 80%. Gotcha: a stream copy can finish before `ffmpeg::run`'s read loop sees a cancel, so run_job has a deterministic cancel check between `Started` and the spawn; `match &events.borrow()[0]` as the last block stmt needs binding to a `let` (Ref temporary lifetime). |
