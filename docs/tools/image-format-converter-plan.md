...
- **2026-05-22 (A4):** Dropped the brief's "if both width/height AND viewBox are missing, the file is UnsupportedFormat" promise. `usvg` defaults to the CSS-spec size (300×150) when nothing is declared, and unconditionally giving a result for a parseable SVG is the friendlier contract. The brief text reflects this; we now only reject SVGs that resolve to a non-positive/non-finite size on their own.
- **2026-05-22 (A4):** `tiny_skia` is accessed via `resvg::tiny_skia` (re-export) rather than added as a direct dep — keeps the dep list narrow and locks tiny_skia to whatever resvg 0.47 wants. If a tool later needs tiny_skia outside the SVG path, that's the moment to add it directly.
