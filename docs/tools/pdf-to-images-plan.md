...
### [x] C9 — test(e2e): pdf-to-images happy path
- **Spec:** `tests/e2e/pdf-to-images.spec.ts` — 1 happy-path Playwright test (~550 ms locally). Flow: dashboard → click "PDF → Images" tile → click "Choose PDF" → assert form visible → click Convert → assert "Open output folder" + "Convert another" buttons appear.
- **Mock strategy (decided):** Vite alias swap rather than per-test runtime mocking. `vite.config.ts` reads `VITE_E2E === "true"` (set by `playwright.config.ts` → `webServer.env`) and remaps `@/lib/tools/pdfToImages` and `@/lib/system` to `tests/e2e/mocks/{pdfToImages,system}.ts`. Production source stays free of test branches; the mocks are typed against the real wrapper signatures (drift → tsc error). Rejected per-test `page.addInitScript` hook approach as overkill for a single smoke test. Per-scenario dynamic control can be layered later via a `window.__E2E_SCENARIO__` flag the mocks consult — not needed for C9.
- **Static mock behaviour:** `pickPdfFile` → resolves with `/tmp/multitool-e2e/sample.pdf`; `convertPdfToImages` → streams 3 progress events at 30 ms each then resolves with `output_dir = "/tmp/multitool-e2e/sample_pages"`; `revealInFolder` → no-op. The cancellation branch is implemented for completeness (mirrors the real wrapper's `Cancelled` envelope) but not exercised by C9's smoke.
- **Adjacent setup work:** `playwright install chromium` runs in fresh environments (CI agents already cache, dev machines need it on first `pnpm test:e2e`); not a script in `package.json` yet — drop in if e2e ergonomics ever bite. Pre-existing skipped `dashboard.spec.ts` left as-is; closes when the dashboard grows beyond the tile list.
- **Tests:** 1 Playwright happy-path. Failure paths (encrypted PDF, cancel, error envelope) are unit-covered in `PdfToImages.test.tsx`.
