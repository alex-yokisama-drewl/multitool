//! IPC types shared between the Tauri shell and pure logic.
//!
//! The job registry maps a UI-generated [`JobId`] to a [`CancellationToken`]
//! so long-running commands can be cooperatively cancelled. The Tauri shell
//! wraps these types in `#[tauri::command]` handlers; tests exercise them
//! directly without any Tauri runtime.
